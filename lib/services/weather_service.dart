...
//   static const String _apiKey = 'a9c99a9cf4a14e649a5213941262304';
...
class WeatherService {
  // 👇 قراءة مفتاح الطقس بأمان من ملف .env
  static final String _apiKey = dotenv.env['WEATHER_API_KEY'] ?? '';

  // ==========================================
  // دالة الذكاء الزراعي (تحليل الطقس وإصدار التحذيرات)
  // ==========================================
  static List<String> analyzeCropWeather({
    required double temp,
    required String condition,
  ...
  // ==========================================
  // دالة مساعدة لجلب الموقع مع خطة طوارئ (Fallback)
  // ==========================================
  static Future<Map<String, double>> _getSafeLocation() async {
    double lat = 30.0444;
    double lon = 31.2357;
  ...
  // 1. الدالة اللي بتشتغل يدوياً عند الضغط على الكبسولة (مع إظهار الـ SnackBar الأحمر)
  static Future<void> checkAndNotifyWeather(BuildContext context) async {
    try {
      if (_apiKey.isEmpty) {
  ...
  // 2. 🔥 الدالة اللي بتشتغل والأبلكيشن مقفول خالص في الخلفية 
  static Future<void> checkAndNotifyWeatherInBackground() async {
    try {
  ...
  // 👇 3. الدالة دي اللي بتعرض في الشاشة 
  static Future<Map<String, dynamic>?> fetchCurrentWeatherForUI() async {
    try {
      if (_apiKey.isEmpty) {
        debugPrint("❌ Weather API Key is missing in .env file!");
